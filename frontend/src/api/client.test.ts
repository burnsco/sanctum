...
function errorJson(status: number, body: Record<string, unknown>, statusText = "Error"): Response {
  return {
    ok: false,
    status,
    statusText,
...
describe("apiClient game room chat", () => {
  ...
  it("fetches game room messages from the expected endpoint", async () => {
    ...
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(okJson(payload));
