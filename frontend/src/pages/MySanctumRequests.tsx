...
export default function MySanctumRequests() {
  const { data = [], isLoading, isError, error, refetch } = useMySanctumRequests();
    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Track approval status.</p>
        </div>
        <Button asChild>
      ...
      <div className="space-y-3">
        {data.length === 0 ? (
          ...
          data.map((request) => (
            <article key={request.id} className="rounded-xl border border-border/70 bg-card/60 p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p className="font-semibold">{request.requested_name}</p>
                  <p className="text-xs text-muted-foreground">/{request.requested_slug}</p>
                </div>
