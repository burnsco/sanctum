import { useAdminDeletedComments, useAdminDeletedPosts } from "@/hooks/useAdminModeration";
...
export default function AdminDeletedContent() {
  ...
  const { data: comments = [], isLoading: commentsLoading } = useAdminDeletedComments({
    limit: 200,
  });
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-2">
        <h2 className="text-base font-semibold">
          Deleted Posts <span className="text-muted-foreground font-normal">({posts.length})</span>
        </h2>
        {postsLoading ? (
                ...
                {posts.map((post) => (
                  <tr key={post.id} className="border-t border-border/60">
                    ...
                    <td className="px-3 py-2">
                      {post.username}{" "}
                      <span className="text-muted-foreground">(#{post.user_id})</span>
                    </td>
                    <td className="px-3 py-2 text-muted-foreground">{post.post_type}</td>
                    <td className="px-3 py-2 text-muted-foreground">{post.sanctum_id ?? "—"}</td>
                    <td className="px-3 py-2 text-muted-foreground">
                      {formatDate(post.created_at)}
                    </td>
                    <td className="px-3 py-2 text-destructive">{formatDate(post.deleted_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                {comments.map((comment) => (
                  <tr key={comment.id} className="border-t border-border/60">
                    ...
                    <td className="px-3 py-2">
                      {comment.username}{" "}
                      <span className="text-muted-foreground">(#{comment.user_id})</span>
                    </td>
                    <td className="px-3 py-2 text-muted-foreground">#{comment.post_id}</td>
                    <td className="px-3 py-2 text-muted-foreground">
                      {formatDate(comment.created_at)}
                    </td>
                    <td className="px-3 py-2 text-destructive">{formatDate(comment.deleted_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
