...
import { useAdminReports, useResolveAdminReport } from "@/hooks/useAdminModeration";
...
export default function AdminReports({ fixedTargetType }: AdminReportsProps) {
  ...
  const [targetFilter, setTargetFilter] = useState<ModerationReportTargetType | "all">(
    fixedTargetType ?? "all",
  );
    ...
    <div className="space-y-4">
      ...
      {!isLoading && !isError && (
        <div className="space-y-3">
          {reports.map((report) => (
            <article key={report.id} className="rounded-xl border border-border/70 bg-card/60 p-4">
