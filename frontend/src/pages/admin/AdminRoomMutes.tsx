...
export default function AdminRoomMutes() {
  ...
  const targetUserId = useMemo(() => Number(targetUserInput || 0), [targetUserInput]);

  const { data: mutes = [], isLoading, isError, refetch } = useChatroomMutes(chatroomId);
  ...
  return (
    <div className="space-y-4">
      <header>
        ...
        <p className="text-sm text-muted-foreground">Manage room-scoped mutes for a chatroom.</p>
      </header>
      ...
      {isLoading && <p className="text-sm text-muted-foreground">Loading room mutes…</p>}
      {isError && (
      ...
      {!isLoading && !isError && (
        <div className="space-y-2">
          {mutes.map((mute) => (
            <article key={mute.id} className="rounded-lg border border-border/70 bg-card/60 p-3">
              ...
              {mute.reason && <p className="mt-1 text-xs text-muted-foreground">{mute.reason}</p>}
            </article>
          ))}
