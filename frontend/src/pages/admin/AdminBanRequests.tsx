...
export default function AdminBanRequests() {
  const { data: requests = [], isLoading, isError, refetch } = useAdminBanRequests({ limit: 200 });
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading && <p className="text-sm text-muted-foreground">Loading ban requests…</p>}
      {isError && (
        <div className="rounded-lg border border-destructive/40 bg-destructive/10 p-3">
          <p className="text-sm text-destructive">Failed to load ban requests.</p>
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                <p className="font-semibold">
                  {request.user?.username || `User #${request.reported_user_id}`}
                </p>
                <Badge variant="outline">{request.report_count} open reports</Badge>
                {request.user?.is_banned && <Badge variant="destructive">Banned</Badge>}
                ...
                <Button
                  ...
                  onClick={() =>
                    banUser.mutate({
                      ...
                      payload: {
                        reason: banReason[request.reported_user_id] || undefined,
                      },
                    })
                  }
                >
