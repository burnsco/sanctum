...
import { useAdminUserDetail, useBanAdminUser, useUnbanAdminUser } from "@/hooks/useAdminModeration";
    ...
    <div className="space-y-4">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Moderation history and account controls.</p>
        </div>
      ...
      {isLoading && <p className="text-sm text-muted-foreground">Loading user detail…</p>}
      {isError && (
        <div className="rounded-lg border border-destructive/40 bg-destructive/10 p-3">
          <p className="text-sm text-destructive">Failed to load user detail.</p>
          <Button size="sm" className="mt-2" variant="outline" onClick={() => refetch()}>
            Retry
          </Button>
        </div>
      )}
        ...
        <>
          <section className="rounded-xl border border-border/70 bg-card/60 p-4">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              <Button size="sm" variant="outline" onClick={() => unbanUser.mutate(data.user.id)}>
                Unban
              </Button>
            </div>
          </section>
          ...
          <section className="rounded-xl border border-border/70 bg-card/60 p-4">
            ...
            <div className="mt-2 space-y-2">
              {data.reports.slice(0, 25).map((report) => (
                <div key={report.id} className="rounded-lg border border-border/60 p-2 text-xs">
                  <p className="font-semibold">
                    #{report.id} • {report.target_type} #{report.target_id} • {report.status}
                  </p>
