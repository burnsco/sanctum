...
export default function SanctumDetail() {
  ...
  const isSanctumOwner = (myMembershipsQuery.data ?? []).some(
    (membership) => membership.sanctum.slug === slug && membership.role === "owner",
  );
  ...
  const [sanctumPosts, setSanctumPosts] = useState<Awaited<ReturnType<typeof apiClient.getPosts>>>(
    [],
  );
  const [sanctumPostsLoading, setSanctumPostsLoading] = useState(false);
  const [sanctumPostsError, setSanctumPostsError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch((error) => {
        if (!cancelled) {
          setSanctumPostsError(error instanceof Error ? error.message : "Failed to load posts");
        }
      })
      .finally(() => {
        if (!cancelled) {
      ...
      <main className="rounded-2xl border border-border/70 bg-card/60 p-4 shadow-xl backdrop-blur-xl md:p-6">
        {sanctumQuery.isLoading ? (
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">{String(sanctumQuery.error)}</p>
            <Button className="mt-3" variant="outline" onClick={() => sanctumQuery.refetch()}>
              Retry
            </Button>
          </div>
        ) : !sanctumQuery.data ? (
          ...
          <>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <Button onClick={() => navigate(`/chat/${sanctumQuery.data.default_chat_room_id}`)}>
                Open Chat
              </Button>
            ...
            <section className="mt-5 rounded-xl border border-border/60 bg-background/50 p-4">
              ...
              {sanctumPostsLoading ? (
                ...
                <p className="mt-2 text-sm text-muted-foreground">No posts in this sanctum yet.</p>
              ) : (
            ...
            {canManageAdmins ? (
              <section className="mt-5 rounded-xl border border-border/60 bg-background/50 p-4">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Manage sanctum-level admins. Role label: "Sanctum Admin" maps to backend role
                  `mod`.
                </p>
                ...
                <div className="mt-4 space-y-2">
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {admin.email} · {admin.role === "mod" ? "Sanctum Admin" : "Owner"}
                        </p>
                      </div>
                      {admin.role === "mod" ? (
                        <Button
                          variant="outline"
                          onClick={() => demoteSanctumAdmin.mutate(admin.user_id)}
                        >
                          Demote
                        </Button>
                      ) : null}
                  ...
                  {sanctumAdminsQuery.isError ? (
                    <p className="text-xs text-destructive">
                      Failed to load sanctum admins: {String(sanctumAdminsQuery.error)}
                    </p>
                  ) : null}
                </div>
              </section>
