...
import { getResumableGameRooms, removeResumableGameRoom } from "@/lib/game-room-presence";
...
export default function Games() {
  ...
  const handlePlayNow = async (type: string) => {
    if (type === "connect4" || type === "othello" || type === "battleship" || type === "checkers") {
      try {
        ...
        if (openRoom) {
          ...
          const myPendingRoom = freshRooms.find(
            (room) =>
              room.status === "pending" && room.creator_id && room.creator_id === currentUser?.id,
          );
          if (myPendingRoom) {
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          <div className="lg:col-span-3 space-y-8">
            <div className="flex items-center gap-2 border-b pb-2">
              ...
              <h2 className="text-xl font-bold uppercase tracking-tight">Live Games</h2>
            </div>
                        ...
                        >
                          <div className="flex flex-col">
                            ...
                            <span className="text-sm font-bold truncate max-w-30">
                              {room.status === "active" ? "In progress" : "Waiting for opponent"}
                            </span>
                          </div>
                          <Button
                            size="sm"
                            ...
                            onClick={() => navigate(`/games/${room.type}/${room.roomId}`)}
                          >
                            Resume
                          </Button>
                        </div>
            ...
            <section className="bg-primary text-primary-foreground rounded-2xl p-6 shadow-lg shadow-primary/20 relative overflow-hidden">
              <div className="relative z-10">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center bg-white/10 p-3 rounded-xl">
                    <span className="text-xs font-bold uppercase opacity-80">Points</span>
