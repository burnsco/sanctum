...
vi.mock("@/hooks/usePresence", () => ({
  usePresenceStore: (selector: (state: { onlineUserIds: Set<number> }) => Set<number>) =>
...
describe("Posts sidebar sanctum links", () => {
  it("renders full sanctum list and highlights active sanctum", () => {
    ...
    expect(within(container).getByRole("link", { name: "Sanctum 5" })).toHaveAttribute(
      "href",
      "/s/sanctum-5",
    );
    expect(within(container).getByRole("link", { name: "Sanctum 1" })).toHaveAttribute(
      "href",
      "/s/sanctum-1",
    );
    expect(within(container).getByRole("link", { name: "Sanctum 1" })).toHaveAttribute(
