...
export default function OnboardingSanctums() {
  ...
  const onSubmit = async () => {
    try {
      ...
      const message = err instanceof Error ? err.message : "Failed to save sanctum choices";
      toast.error(message);
    }
  };
  ...
  if (isError) {
    return (
      <div className="flex h-dvh items-center justify-center">
        <p className="text-destructive">Failed to load sanctums. Refresh and try again.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-dvh flex-col">
      ...
      <div className="shrink-0 border-b border-border/60 px-6 py-6 text-center">
        <h1 className="text-3xl font-bold tracking-tight">Find your communities</h1>
        <p className="mt-2 text-base text-muted-foreground">
          Join sanctums that match your interests. You can always change these later.
        </p>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-base font-semibold leading-snug">{sanctum.name}</p>
                  </div>
