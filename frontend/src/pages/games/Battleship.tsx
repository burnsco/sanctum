...
function ShipVisual({ size }: { size: number }) {
  return (
    <div className="flex flex-row gap-0.5">
      {Array.from({ length: size }, (_, segment) => segment + 1).map((segment) => (
        <div
    ...
    <div className="flex flex-col gap-1">
            ...
            Array.from({ length: GRID_SIZE }).map((_, col) => {
              ...
              return (
                <button
                  ...
                  disabled={myBoard || !gameActive || !isMyTurn || myShots.has(key)}
                ...
                >
                  {isMiss && <div className="h-1.5 w-1.5 rounded-full bg-white/60" />}
                </button>
              );
            }),
          )}
      ...
      <div className="flex gap-0.5 px-1.5">
        {COLUMN_LABELS.map((label) => (
          <div key={label} className="flex-1 text-center text-[8px] text-slate-500 font-mono">
            {label}
          </div>
        ))}
      </div>
...
export default function Battleship() {
  ...
  const {
    ...
    onBoardAction: useCallback((type: string, payload: Record<string, unknown>) => {
  ...
  const myDefenseShips = useMemo((): PlacedShip[] => {
    if (!gameState) return [];
    return isCreator ? (gameState.creator_ships ?? []) : (gameState.opponent_ships ?? []);
  }, [gameState, isCreator]);

  const opponentShipsList = useMemo((): PlacedShip[] => {
    if (!gameState) return [];
    return isCreator ? (gameState.opponent_ships ?? []) : (gameState.creator_ships ?? []);
  }, [gameState, isCreator]);

  const opponentShipCellSet = useMemo(() => shipCellSet(opponentShipsList), [opponentShipsList]);
  ...
  const handleSetupCellClick = (row: number, col: number) => {
    ...
    const newPlaced = [...placedShips.filter((p) => p.name !== selectedShip.name), placed];
  ...
  const overlayState = showVictoryBlast ? "victory" : showDefeatBlast ? "defeat" : null;
    ...
    <div className="h-full overflow-y-auto bg-background text-foreground">
      ...
      <div className="mx-auto grid h-full w-full max-w-300 gap-2 px-2 py-1.5 xl:grid-cols-12 xl:gap-3">
            ...
            <CardHeader className="border-b border-teal-500/10 bg-teal-500/5 px-2 py-1.5 max-[1080px]:px-1.5 max-[1080px]:py-1">
              <div className="grid w-full grid-cols-1 items-center gap-2 md:grid-cols-[1fr_auto_1fr]">
                ...
                <div className="flex items-center gap-2 md:justify-end md:justify-self-end">
                  ...
                  {isPlayer &&
                    (coreState.status === "pending" || coreState.status === "active") && (
                      <Button
                        type="button"
                        variant="ghost"
                        size="icon"
                  ...
                  {shipsSubmitted || amIReady ? (
                    <div className="flex flex-col items-center gap-4 py-8 text-center">
                      ...
                      <p className="text-sm text-muted-foreground font-medium">
                        Waiting for your opponent to finish placing their fleet...
                      </p>
                    </div>
                  ) : (
                      ...
                      <div className="flex shrink-0 flex-col gap-3 xl:w-44">
                          ...
                          return (
                            ...
                            >
                              <div className="flex flex-col gap-0.5">
                                <span className="text-xs font-black">{ship.name}</span>
                        ...
                        <div className="flex flex-col gap-2 mt-2">
                          ...
                          >
                            ...
                            <span className="text-[9px] text-muted-foreground ml-1">[R]</span>
                          </Button>
                          ...
                          >
                            {Array.from({ length: GRID_SIZE }).map((_, row) =>
                              Array.from({ length: GRID_SIZE }).map((_, col) => {
                                ...
                                if (isOccupied) {
                                  cellClass = "bg-teal-700/60 border-teal-600/60 cursor-pointer";
                                } else if (isPreview) {
                                ...
                                return (
                                  <button
                                    ...
                                    onMouseEnter={() => selectedShip && setHoverCell([row, col])}
              ...
              {coreState.status === "finished" && (
                <div className="mt-2 flex flex-col items-center">
                  <div className="mb-4 text-center">
                    ...
                    <p className="text-muted-foreground font-bold">The seas have spoken.</p>
                  </div>
                  <Button
                    variant="outline"
        ...
        <div className="flex min-h-0 flex-col xl:col-span-3 gap-3">
          <Card className="hidden xl:block overflow-hidden border-2 border-teal-500/20 bg-teal-900/10 shadow-lg">
            <div className="h-32 relative">
              <img src={battleshipImg} alt="Battleship" className="w-full h-full object-cover" />
