...
function getJumps(board: Board, row: number, col: number): [number, number][] {
  ...
  for (const [dr, dc] of dirs) {
    ...
    const lr = row + 2 * dr;
    const lc = col + 2 * dc;
    if (inBounds(lr, lc) && isSide(board[mr][mc], opp) && board[lr][lc] === "") {
      jumps.push([lr, lc]);
    }
  }
  return jumps;
...
function getSimpleMoves(board: Board, row: number, col: number): [number, number][] {
  const cell = board[row][col];
  if (!cell) return [];
  const dirs = getDirs(cell);
...
function getSelectablePieces(
  ...
  for (let r = 0; r < BOARD_SIZE; r++) {
    for (let c = 0; c < BOARD_SIZE; c++) {
      if (isSide(board[r][c], side) && getValidMoves(board, r, c, side, null).length > 0) {
        set.add(`${r}-${c}`);
      }
    }
  }
...
function normalizeBoard(rawBoard: unknown, fallback?: Board): Board {
  ...
  return rawBoard.map((row) => {
    ...
    return row.map((cell) => {
      if (cell === "r" || cell === "R" || cell === "b" || cell === "B") return cell as Piece;
      return "" as Piece;
    });
  }) as Board;
}
...
export default function Checkers() {
  ...
  const validMoves = useMemo(() => {
    ...
    const moves = getValidMoves(board, selectedPiece[0], selectedPiece[1], mySide, mustJumpFrom);
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                ...
                                isSelected && isDark && "ring-2 ring-inset ring-amber-300",
                                isSelectable &&
                                  !isSelected &&
                                  isDark &&
                                  "cursor-pointer hover:bg-amber-800/80",
                                isValidMove && "cursor-pointer hover:bg-amber-700/60",
                                !isSelectable && !isValidMove && "cursor-default",
                              )}
                            >
                              {isSide(cell, "r") && (
                                <span
        ...
        <div className="flex min-h-0 flex-col xl:col-span-3 gap-3">
          <Card className="hidden xl:block overflow-hidden border-2 border-amber-500/20 bg-amber-950/10 shadow-lg">
            <div className="h-32 relative">
              <img src={checkersImg} alt="Checkers" className="w-full h-full object-cover" />
