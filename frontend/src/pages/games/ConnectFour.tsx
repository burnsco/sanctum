  ...
  const {
    ...
    onBoardAction: useCallback(
      (type, payload) => {
        ...
        const nextBoard = Array.isArray(rawBoard) ? (rawBoard as string[][]) : createEmptyBoard();
    ...
    <div className="h-full overflow-y-auto bg-background text-foreground">
      ...
      <div className="mx-auto grid h-full w-full max-w-300 gap-2 px-2 py-1.5 xl:grid-cols-12 xl:gap-3">
        <div className="min-h-0 overflow-hidden xl:col-span-9">
          <Card className="flex h-full flex-col border-2 border-blue-500/20 bg-blue-900/10 shadow-xl">
            ...
            <CardContent className="flex min-h-0 flex-1 flex-col items-center justify-center overflow-hidden px-2 py-1.5">
              <div className={`mb-1 grid ${boardWidthClass} grid-cols-7 gap-1.5 px-1.5`}>
                {[...Array(7)].map((_, index) => {
                  const colId = `indicator-${index}`;
                  return (
                    <div key={colId} className="flex h-4 justify-center sm:h-5">
                      {hoverColumn === index && isMyTurn && coreState.status === "active" && (
                    ...
                    row.map((cell, colIndex) => {
                      ...
                      return (
                        <button
                          ...
                          disabled={
                            coreState.status !== "active" || !isMyTurn || board[0][colIndex] !== ""
                          }
              ...
              {coreState.status === "finished" && (
                <div className="mt-2 flex flex-col items-center">
                  <div className="mb-2 text-center">
                    ...
                    <p className="text-muted-foreground font-bold">Hope you had a great vibe!</p>
                  </div>
                  <Button
                    variant="outline"
        ...
        <div className="flex min-h-0 flex-col xl:col-span-3 gap-3">
          <Card className="hidden xl:block overflow-hidden border-2 border-blue-500/20 bg-blue-900/10 shadow-lg">
            <div className="h-32 relative">
              <img src={connect4Img} alt="Connect Four" className="w-full h-full object-cover" />
