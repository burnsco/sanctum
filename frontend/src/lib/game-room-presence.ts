...
function saveRooms(userId: number, rooms: ResumableGameRoom[]) {
  ...
  if (rooms.length === 0) {
    ...
    window.dispatchEvent(new CustomEvent(GAME_ROOM_PRESENCE_EVENT, { detail: { userId } }));
    return;
  }
...
export function removeResumableGameRoom(userId: number | null | undefined, roomId: number) {
  if (!userId) return;
