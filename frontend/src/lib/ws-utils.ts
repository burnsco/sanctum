 ...
 */
export async function createTicketedWS(options: TicketedWSOptions): Promise<WebSocket> {
  let ticketResp: { ticket: string; expires_in: number };
  try {
    ticketResp = await apiClient.issueWSTicket();
    logger.debug("[ws] ticket issued (not logged) expires_in=", ticketResp.expires_in);
  } catch (err) {
 ...
 */
export function getNextBackoff(attempt: number, base = 1000, cap = 30000): number {
