...
describe("getYouTubeEmbedUrl", () => {
  ...
  it("parses watch URLs", () => {
    expect(getYouTubeEmbedUrl(`https://www.youtube.com/watch?v=${videoID}`)).toBe(embedURL);
  });

  it("parses youtu.be URLs with extra params", () => {
    expect(getYouTubeEmbedUrl(`https://youtu.be/${videoID}?si=abc&t=12`)).toBe(embedURL);
  });

  it("parses embed and shorts URLs", () => {
    expect(getYouTubeEmbedUrl(`https://www.youtube.com/embed/${videoID}`)).toBe(embedURL);
    expect(getYouTubeEmbedUrl(`https://www.youtube.com/shorts/${videoID}`)).toBe(embedURL);
  });
