  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const isTimeout = error instanceof Error && error.message.includes("Request timeout");
        ...
        logger.debug(`Token validation failed (network error) in ${duration}ms, assuming valid`);
