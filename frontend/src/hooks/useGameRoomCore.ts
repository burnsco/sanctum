...
export interface UseGameRoomCoreOptions {
  ...
  onBoardAction: (type: GameSocketActionType, payload: GameSocketPayload) => void;
}

function parseStatus(raw: unknown, fallback: GameStatus): GameStatus {
  if (raw === "pending" || raw === "active" || raw === "finished" || raw === "cancelled") {
    return raw;
  }
  return fallback;
}

function buildCancelledState(prev: GameRoomCoreState | null): GameRoomCoreState {
  if (!prev) {
    return {
      status: "cancelled",
      winner_id: null,
...
export function useGameRoomCore({
  ...
  useEffect(() => {
    ...
    const onGameRoomUpdated = (event: Event | CustomEvent<GameRoomRealtimeUpdateDetail>) => {
    ...
    window.addEventListener(GAME_ROOM_REALTIME_EVENT, onGameRoomUpdated as EventListener);

    return () => {
      window.removeEventListener(GAME_ROOM_REALTIME_EVENT, onGameRoomUpdated as EventListener);
    };
    ...
    (action: Record<string, unknown>) => {
      ...
      switch (actionType) {
        ...
        case "game_started": {
            ...
            winner_id:
              typeof payload.winner_id === "number"
                ...
                : payload.winner_id === null
                  ...
                  : (prev?.winner_id ?? null),
            next_turn:
              typeof payload.next_turn === "number" ? payload.next_turn : (prev?.next_turn ?? 0),
            is_draw:
              typeof payload.is_draw === "boolean" ? payload.is_draw : (prev?.is_draw ?? false),
          }));
          ...
          if (actionType === "game_started" && !didShowGameStartedToastRef.current) {
        ...
        case "game_cancelled": {
          const message =
            typeof payload.message === "string" ? payload.message : "A player left this room.";
          void queryClient.invalidateQueries({
            queryKey: ["gameRoom", roomIdParam],
          });
        ...
        case "chat": {
          ...
          const username = typeof payload.username === "string" ? payload.username : "Opponent";
  ...
  const isMyTurn = coreState?.status === "active" && coreState.next_turn === currentUser?.id;
  const didIWin = !!coreState && !coreState.is_draw && coreState.winner_id === currentUser?.id;
  ...
  const handlePlayAgain = useCallback(async () => {
    ...
    try {
      const freshRooms = (await apiClient.getActiveGameRooms(gameType)) as ActiveGameRoom[];
      ...
      const myPendingRoom = freshRooms.find(
        (activeRoom) => activeRoom.status === "pending" && activeRoom.creator_id === currentUser.id,
      );
