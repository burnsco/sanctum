...
describe("useChat hooks", () => {
  ...
  describe("useCreateConversation", () => {
    it("calls API and invalidates conversations", async () => {
      ...
      vi.mocked(apiClient.createConversation).mockResolvedValue(newConvo as never);
  ...
  describe("useMarkAsRead", () => {
    it("calls API with conversation id", async () => {
      ...
      expect(vi.mocked(apiClient).markConversationAsRead).toHaveBeenCalledWith(3);
    });
  });
  ...
  describe("room moderation hooks", () => {
    ...
    it("mutes, kicks, and manages moderators", async () => {
      ...
      const { result: addModResult } = renderHook(() => useAddChatroomModerator(9), {
        wrapper: createWrapper(),
      });
      const { result: removeModResult } = renderHook(() => useRemoveChatroomModerator(9), {
        wrapper: createWrapper(),
      });
