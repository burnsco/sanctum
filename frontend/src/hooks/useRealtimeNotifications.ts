    ...
    (event: MessageEvent) => {
      ...
      switch (data.type) {
        ...
        case "post_reaction_updated": {
          ...
          queryClient.setQueryData<{ likes_count?: number; comments_count?: number } | undefined>(
            ["posts", "detail", postID],
            (oldPost) => {
              ...
              return {
                ...
                comments_count: commentsCount ?? (oldPost.comments_count as number | undefined),
              };
            },
          );
          ...
          for (const query of infiniteQueries) {
            queryClient.setQueryData<InfinitePostsData | undefined>(query.queryKey, (oldData) => {
                  ...
                  page.map((post) =>
                    post.id === postID
                      ? {
                          ...
                          likes_count: likesCount,
                          comments_count:
                            commentsCount ?? (post.comments_count as number | undefined),
                        }
                      : post,
                  ),
                ),
          ...
          queryClient.setQueryData<RealtimeComment[] | undefined>(
            ...
            (oldComments) => {
              ...
              if (data.type === "comment_updated" && comment) {
                return oldComments.map((c) => (c.id === comment.id ? { ...c, ...comment } : c));
              }
        ...
        case "message_received": {
          ...
          const username = asString((payload.from_user as Record<string, unknown>)?.username);
          const preview = asString(payload.preview);
          const conversationId = asNumber(payload.conversation_id);
          const fromUserId = asNumber((payload.from_user as Record<string, unknown>)?.id);
          if (username) {
        ...
        case "chat_mention": {
          ...
          addNotification({
            ...
            description: preview ? `"${preview.slice(0, 120)}"` : "A new mention in chat",
            createdAt: new Date().toISOString(),
            meta: {
              type: "chat_mention",
              userId: fromUserID ?? undefined,
        ...
        case "friend_request_accepted": {
          void queryClient.invalidateQueries({ queryKey: ["friends"] });
          const username = asString((payload.friend_user as Record<string, unknown>)?.username);
          if (username) {
