...
import { useBlockUser, useMyBlocks, useReportUser, useUnblockUser } from "@/hooks/useModeration";
...
export function useUserActions(user: User) {
  ...
  const addFriendLabel =
    status === "pending_sent" || status === "pending_received" ? "Request Sent" : "Add Friend";
