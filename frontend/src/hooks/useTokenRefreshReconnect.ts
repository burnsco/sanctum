  ...
  useEffect(() => {
    ...
    if (!wsEnabled) {
      if (previousTokenRef.current !== currentToken) {
        console.log(`[ws-token-refresh:${label}] websocket disabled, tracking token baseline only`);
      }
    ...
    console.log(`[ws-token-refresh:${label}] token changed, triggering planned reconnect`, {
