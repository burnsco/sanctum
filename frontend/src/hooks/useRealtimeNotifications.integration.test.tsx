...
vi.mock("@/providers/ChatProvider", () => {
  const ChatProviderMock = ({ children }: { children: React.ReactNode }) => <>{children}</>;
  return {
...
describe("realtime notification -> open DM flow", () => {
  ...
  it("seeds the cache correctly so chat can load", async () => {
    ...
    server.use(
      ...
      http.get("/api/auth/me", () => HttpResponse.json({ id: 1, username: "me" })),
      http.get("/api/friends", () => HttpResponse.json([{ id: 2, username: "friend" }])),
    );
    ...
    // Verify cache state
    const cachedList = queryClient.getQueryData(["chat", "conversations"]) as unknown[] | undefined;
    const cachedConv = queryClient.getQueryData(["chat", "conversation", conv.id]);
