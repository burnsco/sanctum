...
describe("useGameRoomSession", () => {
  ...
  it("queues manual join while connecting and sends join after open", async () => {
    ...
    vi.mocked(createTicketedWS).mockResolvedValue(socket as unknown as WebSocket);
    ...
    expect(socket.send).toHaveBeenCalledWith(JSON.stringify({ type: "join_room", room_id: 42 }));
  });
  ...
  it("auto-joins pending room for non-creator on connect", async () => {
    ...
    expect(socket.send).toHaveBeenCalledWith(JSON.stringify({ type: "join_room", room_id: 77 }));
  });
  ...
  it("dispatches incoming messages to the onAction callback", async () => {
    ...
    await act(async () => {
      socket.receive(gameStatePayload);
    });
