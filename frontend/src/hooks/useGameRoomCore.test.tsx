...
describe("useGameRoomCore", () => {
  ...
  beforeEach(() => {
    ...
    mockJoinRoom = mockJoinRoomSpy as unknown as ReturnType<typeof useGameRoomSession>["joinRoom"];
    ...
    mockAddMessage = mockAddMessageSpy as unknown as ReturnType<typeof useGameChat>["addMessage"];
  ...
  function renderCore(overrides: Partial<Parameters<typeof useGameRoomCore>[0]> = {}) {
  ...
  it("maps creator-join errors to user-friendly toast text", async () => {
    ...
    expect(toast.error).toHaveBeenCalledWith("Game Error", {
      description: "Open the room from a different account to join as opponent.",
    });
  });
  ...
  it("suppresses cancel notification when leave was initiated locally", async () => {
    ...
    vi.mocked(apiClient.leaveGameRoom).mockReturnValueOnce(leaveDeferred.promise);
  ...
  it("handlePlayAgain prefers joinable room, then own pending room, then create", async () => {
    ...
    vi.mocked(apiClient.getActiveGameRooms).mockResolvedValueOnce([]);
    vi.mocked(apiClient.createGameRoom).mockResolvedValueOnce(buildRoom({ id: 46 }));
  ...
  it("navigates away when initial room query errors", async () => {
    vi.mocked(apiClient.getGameRoom).mockRejectedValueOnce(new Error("not found"));
