...
describe("useSanctums hooks", () => {
  ...
  describe("useMySanctumRequests", () => {
    it("fetches my requests", async () => {
      ...
      vi.mocked(apiClient).getMySanctumRequests.mockResolvedValue(requests as never);
  ...
  describe("useMySanctumMemberships", () => {
    it("fetches my memberships", async () => {
      ...
      vi.mocked(apiClient).getMySanctumMemberships.mockResolvedValue(memberships as never);
  ...
  describe("useAdminSanctumRequests", () => {
    it("fetches admin requests by status", async () => {
      ...
      vi.mocked(apiClient).getAdminSanctumRequests.mockResolvedValue(requests as never);
      ...
      await waitFor(() => expect(result.current.isSuccess).toBe(true));
      expect(vi.mocked(apiClient).getAdminSanctumRequests).toHaveBeenCalledWith("pending");
    });
  });
  ...
  describe("useCreateSanctumRequest", () => {
    it("calls API with payload", async () => {
      ...
      vi.mocked(apiClient).createSanctumRequest.mockResolvedValue(created as never);
  ...
  describe("useUpsertMySanctumMemberships", () => {
    it("calls API with sanctum slugs", async () => {
      vi.mocked(apiClient).upsertMySanctumMemberships.mockResolvedValue(undefined as never);
      ...
      expect(vi.mocked(apiClient).upsertMySanctumMemberships).toHaveBeenCalledWith({
        sanctum_slugs: ["atrium", "hall"],
      });
    });
  });
  ...
  describe("useApproveSanctumRequest", () => {
    it("calls API with id and optional review_notes", async () => {
      ...
      vi.mocked(apiClient).approveSanctumRequest.mockResolvedValue(response as never);
      ...
      expect(vi.mocked(apiClient).approveSanctumRequest).toHaveBeenCalledWith(1, "Looks good");
    });
  });
  ...
  describe("useRejectSanctumRequest", () => {
    it("calls API with id and optional review_notes", async () => {
      ...
      vi.mocked(apiClient).rejectSanctumRequest.mockResolvedValue(request as never);
      ...
      expect(vi.mocked(apiClient).rejectSanctumRequest).toHaveBeenCalledWith(1, undefined);
    });
  });
});
