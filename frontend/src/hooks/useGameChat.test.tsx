...
describe("useGameChat", () => {
  ...
  it("deduplicates websocket messages when history resolves later", async () => {
    ...
    vi.mocked(apiClient.getGameRoomMessages).mockReturnValueOnce(pending.promise);
  ...
  it("refetches persisted history when remounting the same room", async () => {
    ...
    vi.mocked(apiClient.getGameRoomMessages)
      .mockResolvedValueOnce(buildHistory([{ user_id: 1, username: "a", text: "before-away" }]))
  ...
  it("resets and loads new history when roomId changes", async () => {
    vi.mocked(apiClient.getGameRoomMessages)
      .mockResolvedValueOnce(buildHistory([{ user_id: 1, username: "a", text: "room-1" }]))
      .mockResolvedValueOnce(buildHistory([{ user_id: 2, username: "b", text: "room-2" }]));
    ...
    await waitFor(() => {
      expect(result.current.messages).toEqual([{ user_id: 1, username: "a", text: "room-1" }]);
    });
    ...
    await waitFor(() => {
      expect(result.current.messages).toEqual([{ user_id: 2, username: "b", text: "room-2" }]);
    });
  });
});
