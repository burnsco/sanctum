...
export const chatKeys = {
  ...
  chatroomMutes: (chatroomId: number) => [...chatKeys.chatrooms(), "mutes", chatroomId] as const,
  chatroomBans: (chatroomId: number) => [...chatKeys.chatrooms(), "bans", chatroomId] as const,
};
...
function applyReactionSummaryToMessages(
  ...
  return messages.map((message) =>
    message.id === messageId ? { ...message, reaction_summary: reactionSummary } : message,
  );
}
...
export function useAddChatroomModerator(chatroomId: number) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (userId: number) => apiClient.addChatroomModerator(chatroomId, userId),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: chatKeys.chatroomModerators(chatroomId),
      });
...
export function useRemoveChatroomModerator(chatroomId: number) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (userId: number) => apiClient.removeChatroomModerator(chatroomId, userId),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: chatKeys.chatroomModerators(chatroomId),
      });
  ...
  return useMutation({
    ...
    onSuccess: (_, conversationId) => {
      ...
      queryClient.setQueryData(
        ...
        (old: { unread_count?: number } | undefined) => (old ? { ...old, unread_count: 0 } : old),
      );
      queryClient.setQueryData(
...
export function useAddMessageReaction(conversationId: number) {
  ...
  return useMutation({
    ...
    onSuccess: (response) => {
      queryClient.setQueryData<Message[]>(chatKeys.messages(conversationId), (oldMessages) =>
        applyReactionSummaryToMessages(oldMessages, response.message_id, response.reactions),
      );
    },
    onError: (error) => {
      handleAuthOrFKError(error);
