...
interface UseManagedWebSocketOptions {
  ...
  onError?: (ws: WebSocket, event: Event, meta: ManagedWebSocketCloseMeta) => void;
  onClose?: (ws: WebSocket, event: CloseEvent, meta: ManagedWebSocketCloseMeta) => void;
...
export function useManagedWebSocket({
  ...
  const socketIdByInstanceRef = useRef<WeakMap<WebSocket, number>>(new WeakMap());
  ...
  const [connectionState, setConnectionState] = useState<ManagedWebSocketState>("disconnected");
  ...
  useEffect(() => {
    reconnectDelaysRef.current =
      reconnectDelaysMs.length > 0 ? reconnectDelaysMs : DEFAULT_RECONNECT_DELAYS;
  }, [reconnectDelaysMs]);
  ...
  const handlePingMessage = useCallback((ws: WebSocket, event: MessageEvent) => {
  ...
  const close = useCallback(
    (planned = false) => {
      ...
      wsRef.current = null;
      if (ws.readyState === WS_READY_OPEN || ws.readyState === WS_READY_CONNECTING) {
        markCloseCause(ws, planned ? "close(planned=true)" : "close(planned=false)");
        ws.close();
      }
      setConnectionState("disconnected");
    },
    [clearReconnectTimer, clearHandshakeTimeout, setPlannedReconnect, markCloseCause],
  );
  ...
  const connect = useCallback(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        console.log(formatLogPrefix(socketID), "WebSocket created, readyState=", ws.readyState);
      } catch (err) {
      ...
      ws.onclose = (event) => {
        ...
        const closeCause = socketCloseCauseRef.current.get(socketID) ?? "external-or-peer";
  ...
  const reconnect = useCallback(
    (planned = false) => {
      ...
      const ws = wsRef.current;
      if (ws) {
        markCloseCause(ws, planned ? "reconnect(planned=true)" : "reconnect(planned=false)");
        ws.close();
        return;
      }
  ...
  useEffect(() => {
    ...
    console.log(formatLogPrefix(getSocketID(wsRef.current)), "Enabled changed:", enabled);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const ws = wsRef.current;
      wsRef.current = null;
      if (ws && (ws.readyState === WS_READY_OPEN || ws.readyState === WS_READY_CONNECTING)) {
