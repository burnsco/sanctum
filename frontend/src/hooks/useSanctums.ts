...
export function useCreateSanctumRequest() {
  ...
  return useMutation({
    mutationFn: (payload: CreateSanctumRequestInput) => apiClient.createSanctumRequest(payload),
    onSuccess: () => {
...
export function useSanctumAdmins(slug: string, options?: { enabled?: boolean }) {
  return useQuery({
