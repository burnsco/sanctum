...
import { useAcceptFriendRequest, useFriends, useRejectFriendRequest } from "@/hooks/useFriends";
  ...
  return (
    <DropdownMenuContent align="center" className="w-64">
      <DropdownMenuLabel className="flex items-center justify-between">
        <span>Friends</span>
        <Link to="/friends" className="text-[10px] font-medium text-primary hover:underline">
          View All
        </Link>
      </DropdownMenuLabel>
      <DropdownMenuSeparator />
      <div className="max-h-80 overflow-y-auto">
        {friends.length === 0 ? (
          <div className="px-2 py-4 text-center text-xs text-muted-foreground">No friends yet</div>
        ) : (
          friends.map((friend) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col min-w-0">
                  <span className="truncate text-xs font-medium">{friend.username}</span>
...
export function TopBar() {
  ...
  const unreadCount = useMemo(
    () => displayNotifications.filter((n: (typeof notifications)[0]) => !n.read).length,
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-80">
                ...
                {displayNotifications.length === 0 ? (
                  ...
                  displayNotifications.slice(0, 8).map((item: (typeof notifications)[0]) => (
                    <DropdownMenuItem
                      key={item.id}
                      className={cn(
                    ...
                    >
                      ...
                      <div className="flex w-full items-center justify-between gap-2">
                        ...
                        {item.meta?.type === "friend_request" && item.meta.requestId ? (
                ...
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <Avatar className="h-7 w-7">
                      <AvatarImage src={currentUser.avatar || getAvatarUrl(currentUser.username)} />
                      <AvatarFallback>
