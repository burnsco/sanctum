...
import { useConversation, useConversations, useCreateConversation } from "@/hooks/useChat";
        ...
        <div className="flex items-center border-b border-border/50 bg-muted/30">
          <div className="flex flex-1 items-center gap-1 overflow-x-auto p-1.5 no-scrollbar">
            ...
            {openConversationIds.map((id) => {
              ...
              const isActive = view === "conversation" && activeConversationId === id;
...
export function ChatDock() {
  ...
  const updatePanelSide = useCallback(() => {
    ...
    const canOpenRight = wrapperRect.left + panelWidth <= viewportWidth - viewportPadding;
  ...
  const handleDragStart = useCallback(
    (e: React.MouseEvent) => {
      ...
      const handleMouseMove = (ev: MouseEvent) => {
        ...
        const newX = Math.max(
          0,
          Math.min(ev.clientX - dragOffsetRef.current.x, window.innerWidth - w.offsetWidth),
        );
        const newY = Math.max(
          0,
          Math.min(ev.clientY - dragOffsetRef.current.y, window.innerHeight - w.offsetHeight),
        );
  ...
  const friendUserIds = useMemo(() => new Set(friends.map((f: { id: number }) => f.id)), [friends]);
  ...
  useEffect(() => {
    const unsub = subscribeOnTyping((convId, userId, username, isTyping, expiresInMs) => {
  ...
  useEffect(() => {
    const handleMessage = (message: Message, conversationId: number) => {
      ...
      const isFriendDM =
        friendDMConversationIds.has(conversationId) || friendUserIds.has(message.sender_id);
      ...
      const isCurrentActive =
        conversationId === state.activeConversationId && state.isOpen && !state.minimized;
      ...
      const shouldShowToast =
        !state.isOpen || state.minimized || state.activeConversationId !== conversationId;
      ...
      if (shouldShowToast) {
        ...
        const avatar = conv ? getDirectMessageAvatar(conv, currentUser?.id) : friend?.avatar;
        ...
        >
          ...
          <button
            ...
            className={cn(
              ...
              totalUnread > 0 && "animate-pulse shadow-[0_0_15px_rgba(59,130,246,0.5)]",
            )}
          >
