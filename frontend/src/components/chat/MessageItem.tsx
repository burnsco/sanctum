...
import { useAddMessageReaction, useRemoveMessageReaction } from "@/hooks/useChat";
...
function computeReactionSummary(
  ...
  for (const reaction of reactions) {
    ...
    byEmoji.set(reaction.emoji, {
      ...
      reacted_by_me: previous.reacted_by_me || reaction.user_id === currentUserId,
    });
  }
    ...
    <div className="group relative flex items-start gap-2 py-0.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        {showReadReceipt && isDirectMessage && isOwnMessage && (
          <p className="text-[9px] text-muted-foreground/70">{formatReadReceipt(message)}</p>
        )}
      </div>
    </div>
  );
