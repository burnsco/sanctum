...
function HookTest({ cb }: { cb: (ctx: ReturnType<typeof useChatContext>) => void }) {
...
describe("ChatProvider integration", () => {
  ...
  beforeAll(() => {
    ...
    vi.spyOn(console, "error").mockImplementation((...args: unknown[]) => {
      ...
      if (combined.includes("ChatProvider") && combined.includes("was not wrapped in act")) return;
  ...
  beforeEach(() => {
    ...
    (globalThis as GlobalWithMocks).WebSocket = MockWS as unknown as GlobalWithMocks["WebSocket"];
    MockWS.instances = [];
    qc = new QueryClient();
  ...
  afterEach(() => {
    vi.useRealTimers();
    (globalThis as GlobalWithMocks).WebSocket = originalWS as GlobalWithMocks["WebSocket"];
    MockWS.instances = [];
  ...
  it("records join intent before open, re-joins on connect, and forwards typing events", async () => {
    ...
    function TestHook() {
      ...
      React.useEffect(() => {
        ...
        const unsub = ctx.subscribeOnTyping((convId, userId, username, isTyping) => {
    ...
    expect((capturedCtx as unknown as { joinedRooms: Set<number> }).joinedRooms.has(123)).toBe(
      true,
    );
    ...
    act(() => {
      render(
        <HookTest
          cb={(ctx) => {
            capturedCtx = ctx;
            ctx.subscribeOnMessage((msg: { sender_id?: number }, convId: number) => {
              if ((msg.sender_id ?? 0) === currentUserId) return;
              const nextUnread = ctx.incrementUnread(convId);
              const prevUnread = nextUnread - 1;
              if (shouldPlayFriendDMInMessagesView(conversation, isMessagesRoute, prevUnread)) {
                mockPlayNewMessageSound();
              }
            });
            ctx.subscribeOnPresence((userId: number, _username: string, status: string) => {
              if (
