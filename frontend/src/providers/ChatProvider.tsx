...
interface ChatContextValue {
  ...
  setOnMessage: (callback?: (message: Message, conversationId: number) => void) => void;
  setOnTyping: (
    callback?: (
      conversationId: number,
      userId: number,
  ...
  setOnPresence: (callback?: (userId: number, username: string, status: string) => void) => void;
  ...
  subscribeOnMessage: (callback: (message: Message, conversationId: number) => void) => () => void;
  subscribeOnTyping: (
    callback: (
      conversationId: number,
      userId: number,
  ...
  subscribeOnConnectedUsers: (callback: (userIds: number[]) => void) => () => void;
...
export function ChatProvider({ children }: ChatProviderProps) {
  ...
  useEffect(() => {
    ...
    if (userStr) {
      try {
        const user = JSON.parse(userStr);
        const storageKey = `joined_rooms:${user.id}`;
        localStorage.setItem(storageKey, JSON.stringify(Array.from(joinedRooms)));
      } catch {}
    }
  }, [joinedRooms]);

  // Subscription-based handlers (support multiple subscribers)
  const messageHandlersRef = useRef<Set<(message: Message, conversationId: number) => void>>(
    new Set(),
  );
  ...
  >(new Set());
  const connectedUsersHandlersRef = useRef<Set<(userIds: number[]) => void>>(new Set());
  ...
  useEffect(() => {
    blockedUserIDsRef.current = new Set(myBlocks.map((block) => block.blocked_id).filter(Boolean));
  }, [myBlocks]);
  ...
  const setOnConnectedUsers = useCallback((callback?: (userIds: number[]) => void) => {
    if (!callback) return () => {};
  ...
  const subscribeOnConnectedUsers = useCallback((cb: (userIds: number[]) => void) => {
  ...
  const refreshBlockedUsers = useCallback(async () => {
    try {
      ...
      blockedUserIDsRef.current = new Set(blocks.map((block) => block.blocked_id).filter(Boolean));
    } catch {
  ...
  const handleSocketMessage = useCallback(
    (event: MessageEvent) => {
      try {
         ...
         */
        const applyIncomingMessage = (message: Message, convId: number): boolean => {
          ...
          queryClient.setQueryData<Message[]>(["chat", "messages", convId], (old) => {
            if (!old) return [message];
            if (old.some((m) => m.id === message.id)) return old;

            const tempId = (message.metadata as Record<string, unknown>)?.tempId;
            if (tempId) {
              ...
              if (hasOptimistic) {
                return old.map((m) =>
                  (m.metadata as Record<string, unknown>)?.tempId === tempId ? message : m,
                );
              }
            }
        ...
        switch (data.type) {
          ...
          case "typing": {
            ...
            const expiresInMs = typeof expiresInMsRaw === "number" ? expiresInMsRaw : 5000;
            if (!convId) break;
              ...
              (oldMessages) =>
                oldMessages?.map((message) =>
                  message.id === messageID
                    ? {
                        ...message,
                        reaction_summary: Array.isArray(reactions) ? reactions : [],
                      }
                    : message,
                ) ?? oldMessages,
            );
          ...
          case "read": {
            const payload = data.payload || data;
            const convId = payload.conversation_id || data.conversation_id;
            const readByUserID = payload.user_id || payload.userId || data.user_id;
                  ...
                  oldMessages?.map((message) =>
                    message.sender_id === currentUserID
                      ? {
                          ...message,
                          is_read: true,
                          read_at: typeof readAt === "string" ? readAt : message.read_at,
                        }
                      : message,
                  ) ?? oldMessages,
              );
