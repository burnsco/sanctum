...
test.describe("Sanctum request user flow", () => {
  ...
  test("auth user submits request and sees it in My requests @smoke", async ({ page, request }) => {
    const userToken = readTokenFromStorageState(USER_STATE_PATH);
    ...
    await expect(page.locator("article").filter({ hasText: slug }).first()).toBeVisible({
      timeout: TEST_TIMEOUTS.POLL,
    });
