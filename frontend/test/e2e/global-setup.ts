...
const API_BASE_RAW = process.env.PLAYWRIGHT_API_URL || "http://127.0.0.1:8375/api";
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (attempt < maxAttempts) {
        ...
        const wait =
          Math.min(TEST_TIMEOUTS.RETRY_BASE * attempt, TEST_TIMEOUTS.RETRY_MAX) +
...
async function promoteAdmin(userID: number) {
  ...
  const user = process.env.PGUSER || process.env.DB_USER || process.env.POSTGRES_USER || "user";
  const password =
  ...
  for (const database of candidates) {
    ...
    try {
      ...
      const result = await client.query("UPDATE users SET is_admin = TRUE WHERE id = $1", [userID]);
